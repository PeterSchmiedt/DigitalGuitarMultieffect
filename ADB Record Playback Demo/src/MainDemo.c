...
void InitializeHardware( void )
{
    ...
    ClearDevice(); 		
   	...
   	SetColor(BRIGHTRED);
   	SetFont((void *) &Font25); 
   	OutTextXY(70,65, "AUDIO");
   	OutTextXY(5,65 + GetTextHeight( (void *)&Font25 )-5, " DEVELOPMENT BOARD");
   	SetColor(BRIGHTGREEN);
   	SetFont((void *) &GOLMediumFont); 
   	OutTextXY(25,65 + 2*GetTextHeight( (void *)&Font25 )-10, "www.microchip.com/adb");
...
void CreateScreenLoopback(void)
{
	...
	SetFont((void *) &Font25); 
	SetColor(BRIGHTGREEN);
	OutTextXY(37, 5, "Loopback");  
	SetColor(BRIGHTBLUE);
	OutTextXY(5, 50, "|A|"); 
	OutTextXY(16, 120, "0"); 
	OutTextXY(160, 120, "Fs/2"); 
	SetColor(WHITE);
...
void uitoa2(WORD Value, BYTE* Buffer) {        BYTE i;        WORD Digit;        WORD Divisor;        BOOL Printed = FALSE;         if(Value)        {                for(i = 0, Divisor = 10000; i < 5u; i++)                {                        Digit = Value/Divisor;                        if(Digit || Printed)                        {                                *Buffer++ = '0' + Digit;                                Value -= Digit*Divisor;                                Printed = TRUE;                        }                        Divisor /= 10;                }        }        else        {                *Buffer++ = '0';        }         *Buffer = '\0'; }             

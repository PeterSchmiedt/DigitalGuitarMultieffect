...
int16c frameMatrix[FRAMES][WINDOW_SIZE];
...
void Distortion() {
    ...
    while (i < FRAME_SIZE) {
        ...
        if (Sin[i].rightChannel > treshold) {
            Sin[i].rightChannel = clamp;
        }
        if (Sin[i].rightChannel < -treshold) {
            Sin[i].rightChannel = -clamp;
        }
...
void PitchShift() {
    //calculate the alpha offset
    float alpha = pow(2, (step / 12));
    //calculate the output HOP_SIZE
    INT8 hopSizeOut = floor(alpha * HOP_SIZE);
    ...
    for (currentFrame = 0; currentFrame < FRAMES; currentFrame++) {
        ...
        int currentData;
        for (currentData = 0; currentData < FRAME_SIZE; currentData++) {
            frameMatrix[currentFrame][currentData].re = frameMatrix[currentFrame][currentData].re * hanningWindow[currentData]; //mips_vec_mul16 read the documentation
        }

        //FFT
        //mips_fft16(dout, frameMatrix[currentFrame], (int16c *) fftc, scratch, REAL_LOGN); //mips_fft16(dout, din, (int16c *) fftc, scratch, REAL_LOGN);
    ...
    /*
    char print[30];
    //sprintf(print, "%f", frameMatrix[1][256].re); //384,256,128,0
    sprintf(print, "%f", sizeof(INT16)); //384,256,128,0
    SetColor(WHITE);
    Bar(100, 20, 160, 40);
    SetColor(BLACK);
...
inline void CreateFrames() {
    ...
    for (currentFrame = 0; currentFrame < FRAMES; currentFrame++) {
        int currentData;
        for (currentData = 0; currentData < WINDOW_SIZE; currentData++) {
            if ((currentFrame * HOP_SIZE + currentData - 3 * HOP_SIZE) < 0) {
                frameMatrix[currentFrame][currentData].re = 0;
                frameMatrix[currentFrame][currentData].im = 0;
            } else {
                frameMatrix[currentFrame][currentData].re = Sin[currentFrame * HOP_SIZE + currentData - 3 * HOP_SIZE].leftChannel;
                frameMatrix[currentFrame][currentData].im = 0;
            }
        }
    }
